...
namespace TrainTrain.Test.Acceptance
{
    public class TrainTrainFunctionalCoreShould
    {
            ...
            Reserve_seats_when_train_is_empty()
        {
            ...
            var (provideTrainTopology, provideBookingReference, bookSeats) =
                BuildInputAdapters(TrainTopologyGenerator.With_10_available_seats(), trainNumber, bookingReference,
                    seatsExpected.ToArray());

            // Call imperative Shell

            Check.That(await TrainReservation
                    .ReserveSeats(trainNumber, seatsRequestedCount,
                        provideTrainTopology, provideBookingReference, bookSeats))
                .IsEqualTo(
            ...
            Reserve_all_seats_in_the_same_coach()
        {
            ...
            Check.That(await TrainReservation.ReserveSeats(trainNumber, seatsRequestedCount,
                    provideTrainTopology, provideBookingReference, bookSeats))
                .IsEqualTo(
                    $"{{\"train_id\": \"{trainNumber}\", \"booking_reference\": \"{bookingReference}\", \"seats\": {ReservationAdapter.AdaptSeats(seatsExpected)}}}");
        }
